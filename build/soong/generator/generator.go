...
func init() {
	android.RegisterModuleType("edith_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = edithExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = edithExpandVariables(ctx, dep_file)
	...
	cmd := edithExpandVariables(ctx, String(g.properties.Cmd))
