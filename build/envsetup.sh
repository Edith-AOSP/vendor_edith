...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^edith_") ; then
        EDITH_BUILD=$(echo -n $1 | sed -e 's/^edith_//g')
    else
        EDITH_BUILD=
    fi
    export EDITH_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/edith/vars/aosp_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the edith model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch edith_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/edith-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.edith.device | grep -q "$EDITH_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $EDITH_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.edith.device | grep -q "$EDITH_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.edith.device | grep -q "$EDITH_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $EDITH_FIXUP_COMMON_OUT ]; then
...
function build_kernel() {
    ...
    local edith_version="edith-$(_get_build_var_cached PRODUCT_VERSION_MAJOR).$(_get_build_var_cached PRODUCT_VERSION_MINOR)"
    ...
    if [[ "${SKIP_KERNEL_SYNC}" != "true" && "${SKIP_KERNEL_SYNC}" != "1" ]]; then
        ...
        local repo_init_args=("-b" "${edith_version}")
        if [ -n "${EDITH_MIRROR}" ]; then
            repo_init_args+=("--reference" "${EDITH_MIRROR}")
        fi
        if [ -n "${REPO_VERSION}" ]; then
            repo_init_args+=("--repo-rev" "${REPO_VERSION}")
        fi

        yes | repo init -u https://github.com/Edith-AOSP/${target_kernel_manifest}.git ${repo_init_args[@]} || [ $? -eq 141 ]
        if [ $? -ne 0 ]; then
